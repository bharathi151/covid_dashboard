...
class TestCase03GetStateCumulativeReportDayWiseAPITestCase(CustomAPITestCase):
    app_name = APP_NAME
    ...
    def setupUser(self, username, password):
        super(TestCase03GetStateCumulativeReportDayWiseAPITestCase, self).setupUser(
            username=username, password=password
        )
        mandal = MandalFactory()
        CasesDetailsFactory.create(mandal=mandal, date="2020-06-23",
                                   confirmed_cases=15,
                                   recovered_cases=3,
                                   deaths=0)
